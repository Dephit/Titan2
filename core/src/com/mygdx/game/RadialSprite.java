...
public class RadialSprite implements Drawable {
    ...
    public void draw(Batch batch, float x, float y, float width, float height) {
        ...
        if(isRoundEnded)
            drawEnd(batch, x,  y, width, height);
    }

    private void drawEnd(Batch batch, float x, float y, float width, float height) {
        end.setBounds(x,y,width,height);
        end.setOrigin(end.getWidth()/2,end.getHeight()/2);
        end.setColor(this.color);
        if(this.getAngle()<359 && this.getAngle()>=0)
            end.draw(batch);
    }
